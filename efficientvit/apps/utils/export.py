...
def export_onnx(model: nn.Module, export_path: str, sample_inputs: any, simplify=True, opset=11) -> None:
    ...
    with torch.no_grad():
        ...
        torch.onnx.export(model, sample_inputs, buffer, opset_version=opset,input_names=['input'],  # the model's input names
                      output_names=['classifier.linear'],  # the model's output names
                      dynamic_axes={'input': {0: 'batch_size'},  # variable length axes
                                   'classifier.linear': {0: 'batch_size'}})
        buffer.seek(0, 0)
        if simplify:
