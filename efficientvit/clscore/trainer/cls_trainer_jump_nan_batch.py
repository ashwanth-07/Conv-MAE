...
class ClsTrainer(Trainer):
    ...
    def on_after_backward(self) -> None:
        ...
        for name, param in self.model.named_parameters():
            if param.grad is not None:
                ...
                #     # print('the gradient of "{}" is inf or nan!!!!!'.format(name))
                #     break
    ...
    def _train_one_epoch(self, epoch: int) -> dict[str, any]:
        ...
        with tqdm(
            ...
            for images, labels in self.data_provider.train:
                ...
                t.update()
                if not math.isfinite(train_loss.avg):
                    break
        self.write_log(self.run_config.epoch_format(epoch)+' crashes '+str(crash_time)+' times.', prefix="valid", print_log=False)
        return {
    ...
    def train(self, trials=0, save_freq=1) -> None:
        ...
        for epoch in range(self.start_epoch, self.run_config.n_epochs + self.run_config.warmup_epochs):
            ...
            if math.isfinite(train_info_dict['train_loss']):
                # eval
                ...
                # eval_no_ema
                val_info_dict_noema = self.multires_validate(epoch=epoch, model=self.model)
                avg_top1_noema = list_mean([info_dict["val_top1"] for info_dict in val_info_dict_noema.values()])
                is_best = is_best or avg_top1_noema > self.best_val
                self.best_val = max(avg_top1_noema, self.best_val)
            else:
                avg_top1 = 0.1

            if self.auto_restart_thresh is not None:
                if self.best_val - avg_top1 > self.auto_restart_thresh and self.best_val - avg_top1_noema > self.auto_restart_thresh:
                    ...
                    self.load_model(os.path.join(self.checkpoint_path, str(epoch-1)+".pt"))
            ...
            val_log += f"\tval_top1_no_ema={avg_top1_noema:.2f}({self.best_val:.2f})"
            val_log += "\tVal("
            ...
            self.save_model(only_state_dict=False,epoch=epoch, model_name = str(epoch)+'.pt')